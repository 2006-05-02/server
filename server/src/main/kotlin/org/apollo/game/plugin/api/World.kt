...
fun Region.findObject(position: Position, id: Int): GameObject? {
    return find<GameObject>(position, DYNAMIC_OBJECT, STATIC_OBJECT) { it.type == id }
        .firstOrNull()
}
...
fun Region.findObjects(position: Position, id: Int): Sequence<GameObject> {
    return find(position, DYNAMIC_OBJECT, STATIC_OBJECT) { it.type == id }
}
...
fun World.replaceObject(obj: GameObject, replacement: Int, delay: Int) {
    val replacementObj = DynamicGameObject.createPublic(this, replacement, obj.position, obj.shape, obj.angle)
