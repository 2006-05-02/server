...
object TileUtils {
     ...
     */
    const val TILE_HEIGHT_X_OFFSET: Int = 0xe3b7b

    /**
     * The z coordinate offset, used for computing the Tile height.
     */
    const val TILE_HEIGHT_Z_OFFSET: Int = 0x87cce
    ...
    init {
        for (index in COSINE.indices) {
            COSINE[index] = (65536 * cos(2 * Math.PI * index / COSINE.size)).toInt()
        }
    }
    ...
    private fun computeHeight(x: Int, z: Int): Int {
        ...
        total = max(total * 0.3 + 35, 10.0).toInt()
        return min(total.toDouble(), 60.0).toInt()
    }
    ...
    private fun noise(x: Int, z: Int): Int {
        var n = x + z * 57
        n = (n shl 13) xor n
        n = (n * (n * n * 15731 + 789221) + 1376312589) and Int.MAX_VALUE
